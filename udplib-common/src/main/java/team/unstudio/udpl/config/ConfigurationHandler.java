...
public abstract class ConfigurationHandler {
    ...
    public boolean reload() {
        ...
        for (Field f : getClass().getDeclaredFields()) {
            ...
            setFieldIfConfigValueExist(f, key);
        }
     ...
     */
    private void setFieldIfConfigValueExist(Field f, String key) {
        try {
            f.set(this, config.get(key).orElse(defaults.get(key)));
        } catch (IllegalArgumentException | IllegalAccessException ignored) {
        }
    }
    ...
    public boolean save() {
        ...
        for (Field f : getClass().getDeclaredFields()) {
            ...
            try {
                config.put(key, f.get(this));
            } catch (IllegalArgumentException | IllegalAccessException e) {
                config.put(key, defaults.get(anno.value()));
            }
        }
